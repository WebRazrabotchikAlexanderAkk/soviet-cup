...
<table
	...
	if ( $wishlist && $wishlist->has_items() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					<?php if ( $show_cb ) : ?>
						<td class="product-checkbox">
							<input class="checkbox__input" type="checkbox" value="yes" name="items[<?php echo esc_attr( $item->get_product_id() ); ?>][cb]"/>
					...
					<td class="product-thumbnail ">
						...
						<a class="main-cart__image" href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
					...
					<td class="product-name">
						...
						<a class="main-cart__name" href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							...
							<?php if ( $repeat_remove_button ) : ?>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove_from_wishlist button" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Убрать из избранного', 'yith-woocommerce-wishlist' ) ) ); ?>"><?php esc_html_e( 'Remove', 'yith-woocommerce-wishlist' ); ?></a>
							<?php endif; ?>
					...
					<?php if ( $show_price || $show_price_variations ) : ?>
						<td class="product-price quantity-main-cart__price">
					...
					<?php if ( $show_remove_product ) : ?>
						<td class="product-remove">
							<div>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="favorite__delete remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Убрать из избранного', 'yith-woocommerce-wishlist' ) ) ); ?>"><svg class="inline-svg-icon" width="13" height="13">
		...
		?>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>" class="null-cart__title"><?php echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'Здесь ничего нет. Ваш список избранного пуст', 'yith-woocommerce-wishlist' ), $wishlist ) ); ?></td>
		</tr>
		<?php
	endif;
